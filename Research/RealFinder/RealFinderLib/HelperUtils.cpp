...
/**
 * @brief Render the undistorted result of the given points
 * @param cameraMatrix The camera matrix to use for rendering
 * @param distCoeffs The distortion coefficients to use for rendering
 * @param inputPoints The input points to be rendered
 * @param imageSize The size of the output image
 * @return Mat Returns a Mat
 */
Mat HelperUtils::RenderResult(Mat& cameraMatrix, const Vec4d& distCoeffs, Points * inputPoints, const Size& imageSize) 
{
	Mat image = Mat::zeros(imageSize, CV_8UC3);

	auto undistortedPoints = HelperUtils::Undistort(cameraMatrix, distCoeffs, inputPoints);

	auto& ipoints_1 = undistortedPoints->GetImagePoints_1();
	auto& ipoints_2 = undistortedPoints->GetImagePoints_2();
