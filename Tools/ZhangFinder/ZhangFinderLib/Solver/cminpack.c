...
void fdjac2(void f(int,int,double *,double *,int *,void *),int m,int n,double x[],double fvec[],double **fjac,
						int *iflag,double epsfcn,double wa[], void * callback)
{
	int i,j;
...
void lmdif(void f(int,int, double*,double*,int*,void*),int m,
					 ...
					 double wa3[],double wa4[],void *callback)
{
	...
	*nfev = 1;
	...
	while(1) {
		...
		iflag = 2;
		fdjac2(f,m,n,x,fvec,fjac,&iflag,epsfcn,wa4,callback);
		*nfev += n;
		if (iflag < 0) {
			*info = iflag;
			return;
...
int lmdif0(void fcn(int,int, double*,double*,int*,void*),int m, int n,double x[],int msk[],
					 double fvec[],double tol,int *info,int *nfev,void *callback)
{
	int j,maxfev,mode;
	int *ipvt;
	...
	*nfev = 0;
	...
	lmdif(fcn,m,n,x,msk,fvec,ftol,xtol,gtol,maxfev,epsfcn,diag,mode,
		factor,info,nfev,fjac,ipvt,qtf,wa1,wa2,wa3,wa4,callback);
