...
namespace NVL_App
{
	...
	private:
		...
		Vec3d _tvec;
	public:
		Arguments(Size& gridSize, int blockSize, Size& imageSize, Vec4d& distortion, Vec2d& center, double baseLine, Vec3d& rvec, Vec3d& tvec) :
			_gridSize(gridSize), _blockSize(blockSize), _imageSize(imageSize), _distortion(distortion), _center(center), _baseLine(baseLine), _rvec(rvec), _tvec(tvec) {}
